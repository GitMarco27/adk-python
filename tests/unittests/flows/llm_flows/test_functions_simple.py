...
@pytest.mark.asyncio
async def test_duplicate_transfer_to_agent_calls_rejected():
  """Test that duplicate transfer_to_agent calls return error responses."""
  from google.adk.flows.llm_flows.functions import handle_function_call_list_async

  transfer_executed = []

  def mock_transfer(agent_name: str, tool_context: ToolContext) -> None:
    transfer_executed.append(agent_name)
    tool_context.actions.transfer_to_agent = agent_name

  # Create multiple transfer_to_agent calls
  function_calls = [
      types.FunctionCall(
          id="call_1", name="transfer_to_agent", args={"agent_name": "agent_a"}
      ),
      types.FunctionCall(
          id="call_2", name="transfer_to_agent", args={"agent_name": "agent_b"}
      ),
      types.FunctionCall(
          id="call_3", name="transfer_to_agent", args={"agent_name": "agent_c"}
      ),
  ]
  ...
  agent = Agent(
      ...
      tools=[FunctionTool(func=mock_transfer)],
  )
  ...
  tools_dict = {"transfer_to_agent": FunctionTool(func=mock_transfer)}

  result = await handle_function_call_list_async(
      invocation_context=invocation_context,
      function_calls=function_calls,
      tools_dict=tools_dict,
  )

  # Verify only the first transfer was executed
  assert len(transfer_executed) == 1
  assert transfer_executed[0] == "agent_a"

  # Verify the result event has responses for all calls
  assert result is not None
  function_responses = result.get_function_responses()
  assert len(function_responses) == 3

  # First call should succeed
  first_response = next(r for r in function_responses if r.id == "call_1")
  assert "error" not in first_response.response

  # Second and third calls should have error responses
  second_response = next(r for r in function_responses if r.id == "call_2")
  assert "error" in second_response.response
  assert "cannot be called multiple times" in second_response.response["error"]

  third_response = next(r for r in function_responses if r.id == "call_3")
  assert "error" in third_response.response
  assert "cannot be called multiple times" in third_response.response["error"]

  # Verify only one transfer_to_agent action is set (from the first call)
  assert result.actions.transfer_to_agent == "agent_a"
